...
goog.string.compareVersions = function(a, b) {
  for(var c = 0, d = goog.string.trim("" + a).split("."), e = goog.string.trim("" + b).split("."), f = Math.max(d.length, e.length), g = 0;0 == c && g < f;g++) {
    ...
    do {
      ...
      var c = 0 == k[1].length ? 0 : parseInt(k[1], 10), n = 0 == l[1].length ? 0 : parseInt(l[1], 10), c = goog.string.compareElements_(c, n) || goog.string.compareElements_(0 == k[2].length, 0 == l[2].length) || goog.string.compareElements_(k[2], l[2])
    }while(0 == c)
  }
  return c
};
...
function getPreferenceFromProfile(a, b) {
  ...
  if(!c.prefHasUserValue(a)) {
    return fxdriver.Logger.dumpn(a + " not set; defaulting to " + b), b
  }
  c = c.getBoolPref(a);
  fxdriver.Logger.dumpn("Found preference for " + a + ": " + c);
  return c
}
...
var WdCertOverrideService = function() {
  ...
  this.default_bits = shouldAssumeUntrustedIssuer() ? this.ERROR_UNTRUSTED : 0;
...
WdCertOverrideService.prototype.certificateExpiredBit_ = function(a, b) {
  return 0 != (b & a.CERT_EXPIRED) ? (fxdriver.Logger.dumpn("Certificate expired."), this.ERROR_TIME) : 0
};
WdCertOverrideService.prototype.certificateIssuerUntrusted_ = function(a, b) {
  return 0 != (b & a.ISSUER_UNKNOWN) || 0 != (b & a.ISSUER_NOT_TRUSTED) || 0 != (b & a.CERT_NOT_TRUSTED) || 0 != (b & a.INVALID_CA) ? (fxdriver.Logger.dumpn("Certificate issuer unknown."), fxdriver.Logger.dumpn("Unknown: " + (a.ISSUER_UNKNOWN & b)), fxdriver.Logger.dumpn("Issuer not trusted: " + (a.ISSUER_NOT_TRUSTED & b)), fxdriver.Logger.dumpn("Cert not trusted: " + (a.CERT_NOT_TRUSTED & b)), fxdriver.Logger.dumpn("Invalid CA: " + (a.INVALID_CA & b)), this.ERROR_UNTRUSTED) : 0
};
WdCertOverrideService.prototype.certificateHostnameMismatch_ = function(a, b) {
  var c = RegExp("^" + a.commonName.replace("*", "[\\w|-]+") + "$", "i");
  return null === b.match(c) ? (fxdriver.Logger.dumpn("Host name mismatch: cert: " + a.commonName + " get: " + b), this.ERROR_MISMATCH) : 0
};
WdCertOverrideService.prototype.fillNeededBits = function(a, b) {
  ...
  fxdriver.Logger.dumpn("Certificate verification results: " + c);
  d |= this.certificateExpiredBit_(a, c);
  d |= this.certificateHostnameMismatch_(a, b);
  0 == d && (fxdriver.Logger.dumpn("Checking issuer since certificate has not expired or has a host name mismatch."), d |= this.certificateIssuerUntrusted_(a, c));
  fxdriver.Logger.dumpn("return_bits now: " + d);
  return d
};
WdCertOverrideService.prototype.hasMatchingOverride = function(a, b, c, d, e) {
  var f = !1;
  !0 === this.acceptAll ? (fxdriver.Logger.dumpn("Allowing certificate from site: " + a + ":" + b), f = !0, e.value = !1, d.value = this.fillNeededBits(c, a), fxdriver.Logger.dumpn("Override Bits: " + d.value)) : f = this.origListener_.hasMatchingOverride(a, b, c, d, e);
  return f
};
...
var CERTOVERRIDE_CONTRACT_ID = "@mozilla.org/security/certoverride;1", DUMMY_CERTOVERRIDE_SERVICE_CLASS_ID = Components.ID("{c8fffaba-9b7a-41aa-872d-7e7366c16715}"), service = void 0, WDCertOverrideFactory = {createInstance:function(a) {
  if(null != a) {
...
WDBadCertListenerModule.prototype.unregisterSelf = function(a, b) {
...
var FACTORY;
bot.userAgent.isProductVersion("10") || (FACTORY = {createInstance:function(a) {
  if(null != a) {
    throw CR.NS_ERROR_NO_AGGREGATION;
  }
  if(void 0 != service) {
  ...
  var a = new WdCertOverrideService, b = CC["@mozilla.org/thread-manager;1"].getService().mainThread;
  return service = CC["@mozilla.org/xpcomproxy;1"].getService(CI.nsIProxyObjectManager).getProxyForObject(b, CI.nsICertOverrideService, a, 5)
...
WDBadCertListenerModule.prototype.canUnload = function() {
  return true
};
