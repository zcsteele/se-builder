...
Dispatcher.translateNewSession = function() {
  ...
  return function(a, b) {
    Components.classes["@googlecode.com/webdriver/command-processor;1"].getService(Components.interfaces.nsICommandProcessor).execute('{"name":"newSession"}', function(c) {
      var d = JSON.parse(c);
      0 != d.status ? b.sendError(Response.INTERNAL_ERROR, c, "application/json") : (c = a.getRequestUrl(), b.setStatus(Response.SEE_OTHER), b.setHeader("Location", c.scheme + "://" + c.host + ":" + c.hostPort + c.path + "/" + d.value), b.commit())
    })
  }
};
Dispatcher.prototype.init_ = function() {
  ...
  this.bind_("/session").on(Request.Method.POST, Dispatcher.translateNewSession());
  ...
  this.bind_("/session/:sessionId/alert").on(Request.Method.GET, Dispatcher.executeAs("getAlert"));
  ...
  this.bind_("/session/:sessionId/alert_text").on(Request.Method.GET, Dispatcher.executeAs("getAlertText"));
  ...
  this.bind_("/session/:sessionId/log").on(Request.Method.POST, Dispatcher.executeAs("getLogs"));
...
Dispatcher.prototype.dispatch = function(a, b) {
  ...
  if(0 != c.indexOf("/hub")) {
    ...
    for(var c = a.getPathInfo(), d, e = 0;e < this.resources_.length;e++) {
      if(this.resources_[e].isResourceFor(c) && (!d || d.getNumVariablePathSegments() < this.resources_[e].getNumVariablePathSegments())) {
        d = this.resources_[e]
      }
    }
    if(d) {
      try {
        ...
        fxdriver.Logger.dump(f), b.sendError(Response.INTERNAL_ERROR, JSON.stringify({status:bot.ErrorCode.UNKNOWN_ERROR, value:fxdriver.error.toJSON(f)}), "application/json")
      }
    }else {
...
fxdriver.modals.acceptAlert = function(a) {
  ...
  b.close(); // qqDPSWD Somehow, clicking the accept button erases the text of the alert but does not close it. So we manually close it here.
...
WebDriverServer.prototype.newDriver = function(a) {
  if(null == this.useNativeEvents) {
    ...
    b.prefHasUserValue("webdriver_enable_native_events") || fxdriver.Logger.dumpn("webdriver_enable_native_events not set; defaulting to false");
    this.enableNativeEvents = b.prefHasUserValue("webdriver_enable_native_events") ? b.getBoolPref("webdriver_enable_native_events") : !1;
...
WebDriverServer.prototype.createInstance = function() {
  this.startListening(WebDriverServer.readPort())
};
