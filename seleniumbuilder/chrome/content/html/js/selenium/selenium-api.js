   ...
   */
    ...
    if (browserVersion.isChrome && elementWithHref != null) {
        ...
        if (savedEvent && savedEvent.getPreventDefault()) {
          ...
          while (curElem && html && curElem !== html) {
    ...
    var popupLoadedPredicate = function () {
        ...
        try {
            if (windowID && windowID != 'null') {
                targetWindow = browserbot.getWindowByName(windowID, true);
            }
            else {
                var names = browserbot.getNonTopWindowNames();
                targetWindow = browserbot.getWindowByName(names[0], true);
            }
        }
        catch (e) {
            if (new Date().getTime() > timeoutTime) {
        ...
        if (browserVersion.isSafari) {
            if(targetWindow.location.href == browserbot.buttonWindow.location.href) {
        ...
        if (!browserbot.getCurrentWindow().document.readyState) {
...
Selenium.prototype.getAttributeFromAllWindows = function(attributeName) {
    ...
    */
    ...
    {
        try {
            win = this.browserbot.openedWindows[windowName];
            attributes.push(eval("win."+attributeName));
...
Selenium.prototype.findWindow = function(soughtAfterWindowPropertyValue) {
   ...
   for (windowName in this.browserbot.openedWindows) {
       var openedWindow = this.browserbot.openedWindows[windowName];
...
Selenium.prototype.doWaitForCondition = function(script, timeout) {
   ...
   * the JavaScript snippet <code>this.browserbot.getCurrentWindow()</code>, and then
